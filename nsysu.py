...
def get_key(session):
    ...
    key = js_text[js_text.find('var _key = "') +
                  12:js_text.find("function chec")-4]
    return key
...
def selcrs2_login(session, username, password):
    ...
    url = 'http://selcrs.nsysu.edu.tw/menu4/Studcheck_sso2.asp'
    data = {
        'stuid': username,
        'SPassword': load_js.call('base64_md5', password)
    }
    res = session.post(url=url, data=data)
...
if __name__ == "__main__":
    ...
    selcrs2_login(session=sess, username='', password='')
